...
static const struct gpio_dt_spec magnet_pin =
    GPIO_DT_SPEC_GET(DT_ALIAS(magnet_pin), gpios);

static struct gpio_callback magnet_cb_data;

/* Callback function on pin interrupt */
static void magnet_pin_cb(const struct device *dev, struct gpio_callback *cb,
                          uint32_t pins)
{
    printk("Here\n");
}
...
int initialise_magnet_sensor(void)
{
    ...
    if (ret != 0) {
        printk("Error %d: failed to configure %s pin %d\n", ret,
               magnet_pin.port->name, magnet_pin.pin);
        return ret;
    }
